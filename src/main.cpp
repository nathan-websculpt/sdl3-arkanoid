...
int main()
{
    ...
    while (running)
    {
        ...
        const SDL_FRect ballRect{state.ball.x - (kBallSize * 0.5f),
                                 state.ball.y - (kBallSize * 0.5f), kBallSize, kBallSize};
        ...
        {
            if (!SDL_SetRenderDrawColor(renderer.get(), countdownRed, countdownGreen, countdownBlue,
                                        255))
            {
                return 1;
            }
